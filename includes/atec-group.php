...
<div class="atec-page">';
	...
	<div class="atec-main">';
		...
		if ($license=='true')
		{
			atec_little_block(__('atec-Plugins „PRO“ package','atec-debug'));
			echo '
			<div class="atec-g atec-border atec-center" style="padding: 20px 10px;">
				<h3 class="atec-mt-0">
					...
					esc_attr__('atec-Plugins „PRO“ package','atec-debug'), 
				'</h3>
				<div id="atec_group_pro_package">
					<div id="pro_package_welcome" class="atec-fit" style="margin: 0 auto;">
						<div class="atec-mt-5 atec-border-white atec-bg-w atec-fit" style="font-size: 16px !important; padding: 10px;">
							<ul class="atec-m-0">
							<li>⭐ ', esc_attr__('18 valuable plugins','atec-debug'), '.</li>					
							<li>⭐ ', esc_attr__('Access to all the „PRO“ features','atec-debug'), '.</li>
							<li>⭐ ', esc_attr__('„Lifetime-site-License“ for your site (domain)','atec-debug'), '.</li>
							</ul>
						</div>
					</div>
				<br>';
			...
			<button class="button button-primary">', esc_attr__('GET YOUR „PRO“ PACKAGE NOW','atec-debug'), '</button></a>
			...
			echo '<p styl="font-size: 18px !important;">',
				esc_attr__('Buy the „PRO“ package through one time payment','atec-debug'), '.<br>',
				esc_attr__('The license is valid for the lifetime of your site (domain)','atec-debug'), '.<br><b>',
				esc_attr__('No subscription. No registration required.','atec-debug'), '</b>
			</p>
			</div>';
		...
		<div class="atec-g">
			...
			<thead>
				...
				<th></th>
				<th>Name (Link)</th>
				<th>', esc_attr__('Size','atec-debug'), '</th>
				<th>Status</th>
				<th>', esc_attr__('Preview','atec-debug'), '</th>
				<th>', esc_attr__('Installed','atec-debug'), '</th>
				<th>', esc_attr__('Description','atec-debug'), '</th>
				</tr>
			</thead>
			<tbody>';
		...
		$atec_review			= ['debug'];
		$atec_desc_arr		= [
							__('All-in-one Backup and restore solution – fast & reliable','atec-debug'),
							__('APCu object and page cache','atec-debug'),
							__('atec Cache Info & Statistics (OPcache, WP-object-cache, JIT, APCu, Memcached, Redis, SQLite-object-cache)','atec-debug'),
							__('Custom code snippets for WP','atec-debug'),	
							__('Optimize WP database tables','atec-debug'),

							__('Show debug log in admin bar','atec-debug'),																	
							__('Install and auto update `atec´ plugins','atec-debug'),
							__('Dir Scan & Statistics (Number of files and size per directory)','atec-debug'),
							__('Add custom meta tags to the head section','atec-debug'),
							__('Lightweight performance tuning plugin','atec-debug'),

							__('Measure the PageScore and SpeedIndex of your WordPress site','atec-debug'),
							__('Custom translation strings for polylang plugin','atec-debug'),
							__('Measure plugins & theme plus pages execution time','atec-debug'),	
							__('Add custom SMTP mail settings to WP_Mail','atec-debug'),	
							__('Lightweight and GDPR compliant WP statistics','atec-debug'),				

							__('System Information (OS, server, memory, PHP and database details, php.ini, wp-config, .htaccess and PHP extensions)','atec-debug'),
							__('Manage WordPress User Roles and Capabilities','atec-debug'),
							__('Web map, conform with privacy regulations','atec-debug'),
							__('Auto convert all images to WebP format','atec-debug')
						];
		...
		foreach ($atec_group_arr as $a)
		{
			...
			echo '<tr>
				...
				if ($isWP) echo '
					<td><span title="', esc_attr__('Published','atec-debug'), '" class="',esc_attr(atec_dash_class('wordpress')), '"></span></td>
				...
				else 
				{
					...
					echo '
					<td colspan="2">
						<span title="', $inReview?esc_attr__('In review','atec-debug'):esc_attr__('In progress','atec-debug'), '"><span class="',esc_attr(atec_dash_class($inReview?'visibility':'')) ,'"></span>
					</td>';
				}
		...
		<center>
			<p class="atec-fs-10" style="max-width:80%;">',
				esc_attr__('All our plugins are optimized for speed, size and CPU footprint (frontend & backend)','atec-debug'), '.<br>',
				esc_attr__('Also, they share the same `atec-WP-plugin´ framework – so that shared code will only load once, even with multiple plugins enabled','atec-debug'), '.	<br>',
				esc_attr__('With an average of 1 ms CPU time, we consider our plugins to be super fast – other plugins typically require much more time','atec-debug'), '.<br>',
				esc_attr__('OSs supported: Linux (CloudLinux, Debian, Ubuntu), Windows & Mac-OS','atec-debug'), '.<br>',
				esc_attr__('Webservers tested: Apache, NGINX & LiteSpeed','atec-debug'), '.
			</p>
			<a class="atec-nodeco" class="button atec-center" href="https://de.wordpress.org/plugins/search/atec/" target="_blank">', esc_attr__('All atec-plugins in the WordPress directory','atec-debug'), '.</a>
		</center>';
	}
